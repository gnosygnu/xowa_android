...
function Xo__file_browser__get_selected_chk(url_base, chk_prefix) { // XOWA
  ...
  bridge.sendMessage( 'linkClicked', { "href": url_base + selected } );
}
...
document.onclick = function() {
    ...
    if (sourceNode) {
        if ( sourceNode.hasAttribute( "data-action" ) ) {
            ...
            } else if (stringStartsWith(href, "javascript:")) {
...
bridge.registerListener("xowa__img__update", function ( payload ) {
//    alert(payload.img_id + "|" + payload.img_src + "|" + payload.img_w.toString() + "|" + payload.img_h.toString());
    var img = document.getElementById(payload.img_id);
    if (img == null) return;
    img.src = payload.img_src;
    img.width = payload.img_w;
    img.height = payload.img_h;
...
function runMathjax() { // XOWA
    ...
    if (window.mathJax_Config == null) {  // hookup math; needed if math is not loaded by page; EX: Go to home/wiki/Help:Diagnostics and hover over "Help:Diagnostics/Math"
      window.mathJax_Config = function () {
        ...
        , extensions: ['wiki2jax.js','toMathML.js']
      ...
      script2.setAttribute( 'src', 'file:///android_asset/xowa/bin/any/xowa/xtns/Math/modules/mathjax/android/MathJax.js');
...
transformer.register( "hideTables", function( content ) {
    ...
    for (var i = 0; i < tables.length; i++) {
        ...
        tables[i].style.display = 'block';  // XOWA
        collapsedDiv.style.display = 'block';// XOWA
        bottomDiv.style.display = 'block';// XOWA
