 ...
 * - and many handlers.
 */
public class OfflinePageLoadStrategy implements PageLoadStrategy {
    ...
    public static Xod_app_mgr xo_app_mgr;   // XOWA

    public OfflinePageLoadStrategy() {
    ...
    public void onActivityCreated(@NonNull List<PageBackStackItem> backStack) {
        ...
        Xod_app_mgr.Instance.Rebind_message_handlers(bridge);
...
//        Page page = model.getPage();
    ...
    private JSONObject leadSectionPayload(Page page) {
        ...
        JSONObject section = page.getSections().size() == 0 ? new JSONObject() : page.getSections().get(0).toJSON();
        try {
            return new JSONObject()
