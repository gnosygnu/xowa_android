...
public class Xob_siteinfo_parser_ {
	...
	public static Xob_siteinfo_nde Parse(String xdoc_src, Xow_ns_mgr ns_mgr) {
		...
		for (int i = 0; i < root_len; ++i) {
			...
			else if	(String_.Eq(sub_name, "ba"+"se"))		main_page = Parse_base(Bry_.new_u8(sub_nde.Text_inner()));
			else if (String_.Eq(sub_name, "name"+"spaces"))	Parse_namespaces(sub_nde, ns_mgr, case_dflt);
