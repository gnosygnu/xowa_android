...
class Srch_temp_tbl_wkr implements Srch_text_parser_wkr {
	...
	private static void Search_link__insert(Srch_db_mgr search_db_mgr, Db_conn word_conn, Db_conn page_conn) {
		...
		page_conn.Meta_idx_create(Dbmeta_idx_itm.new_normal_by_name("page", "page_ns__page_id", "page_name"+"space", "page_id"));
		Srch_db_mgr.Optimize_unsafe_(word_conn, Bool_.Y);
		word_conn.Meta_tbl_remake(Dbmeta_tbl_itm.New("search_link_temp", Dbmeta_fld_itm.new_int("word_id"), Dbmeta_fld_itm.new_int("page_id"), Dbmeta_fld_itm.new_int("page_name"+"space")));
		...
		( "INSERT INTO search_link_temp (word_id, page_id, page_name"+"space)"
		, "SELECT  w.word_id"
		, ",       t.page_id"
		, ",       p.page_name"+"space"
		, "FROM    search_temp t"
		...
		word_conn.Meta_idx_create(Dbmeta_idx_itm.new_normal_by_name("search_link_temp", "main", "page_name"+"space", "word_id", "page_id"));
		Srch_db_mgr.Optimize_unsafe_(word_conn, Bool_.N);
		...
		for (int i = 0; i < len; ++i) {
			...
			, "WHERE   t.page_name"+"space" + (i == 0 ? " = 0" : " != 0")
			));
			link_tbl.Create_idx__page_id();
