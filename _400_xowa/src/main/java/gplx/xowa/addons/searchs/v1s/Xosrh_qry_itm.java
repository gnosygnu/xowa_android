...
class Xosrh_qry_itm {
	...
	private static List_adp Search_word(Xowe_wiki wiki, Cancelable cxl, Bry_bfr tmp_bfr, Srch_ns_mgr ns_mgr, byte[] search_word, int results_max) {
		...
		wiki.Db_mgr().Load_mgr().Load_search(cxl, found, search_word, results_max);
	...
	public void Search(Cancelable cxl, Bry_bfr tmp_bfr, byte[] src, Xowe_wiki wiki, int results_max, Srch_ns_mgr ns_mgr) {
		if (cxl.Canceled()) {cxl.Cancel_ackd_(); return;}
		switch (tid) {
			case Xosrh_qry_itm.Tid_null: return;
			case Xosrh_qry_itm.Tid_word:
				ids = Search_word(wiki, cxl, tmp_bfr, ns_mgr, word, results_max);
				break;
			case Xosrh_qry_itm.Tid_word_quote:
				Ordered_hash tmp_search_list = Ordered_hash_.New();
				byte[][] words = gplx.xowa.bldrs.cmds.texts.tdbs.Srch_bldr_wkr_base.Split_ttl_into_words(wiki.Lang(), tmp_search_list, tmp_bfr, word);
				int words_len = words.length;
				List_adp prv_list = null;
				for (int i = 0; i < words_len; i++) {
					if (cxl.Canceled()) {cxl.Cancel_ackd_(); return;}
					List_adp cur_list = Search_word(wiki, cxl, tmp_bfr, ns_mgr, words[i], results_max);
				...
				if (ids_len == 0) return;
				if (cxl.Canceled()) {cxl.Cancel_ackd_(); return;}
				wiki.Db_mgr().Load_mgr().Load_by_ids(cxl, tmp_ids, 0, ids_len);
				if (cxl.Canceled()) {cxl.Cancel_ackd_(); return;}
				ids = List_adp_.new_();
				...
				break;
			default:
				if (lhs != null) lhs.Search(cxl, tmp_bfr, src, wiki, results_max, ns_mgr);
				if (rhs != null) rhs.Search(cxl, tmp_bfr, src, wiki, results_max, ns_mgr);
				break;
		}
	}
