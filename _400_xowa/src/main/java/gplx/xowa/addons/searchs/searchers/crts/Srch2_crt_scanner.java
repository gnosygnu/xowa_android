...
class Srch2_crt_scanner {
	...
	public Srch2_crt_scanner(Srch2_text_parser parser) {this.parser = parser;}
	public Srch2_crt_tkn[] Scan(byte[] src) {
		...
		while (pos < src_len) {
			...
			if (cur_obj == null) {					// text character
				...
				if (txt_bgn != -1) {				// pending word; create
					Tkns_add_word(Srch2_crt_tkn.Tid_word, txt_bgn, pos);
					txt_bgn = -1;
				}
				switch (cur_tid) {
					case Srch2_crt_tkn.Tid_space:	// discard spaces
						pos = Bry_find_.Find_fwd_while(src, pos, src_len, Byte_ascii.Space);
						break;
					case Srch2_crt_tkn.Tid_quote:	// find end quote and add as word
						int quote_bgn = pos + 1;
						...
						Tkns_add_word(Srch2_crt_tkn.Tid_word_quoted, quote_bgn, quote_end);
						pos = quote_end + 1;		// +1 to place after quote
						break;
					case Srch2_crt_tkn.Tid_not: 
						Tkns_add_word(Srch2_crt_tkn.Tid_not, pos, pos_end);
						pos = pos_end;
						break;
					case Srch2_crt_tkn.Tid_paren_bgn: case Srch2_crt_tkn.Tid_paren_end:
					case Srch2_crt_tkn.Tid_and: case Srch2_crt_tkn.Tid_or:
		...
		return (Srch2_crt_tkn[])tkns.To_ary_and_clear(Srch2_crt_tkn.class);
	}
	private boolean Cur_join_is_word(byte cur_tid, int pos_end) {	// extra logic to handle and / or occuring in unquoted strings; EX: "random"; "for"
		switch (cur_tid) {
			...
			case Srch2_crt_tkn.Tid_and: case Srch2_crt_tkn.Tid_or: case Srch2_crt_tkn.Tid_not:
				break;
		}
		boolean join_is_word = true;
		if (txt_bgn == -1) {		// no pending word;
			if (cur_tid == Srch2_crt_tkn.Tid_not) return false;	// NOT is only operator if no pending tkn; EX: -abc -> NOT abc; a-b -> a-b
			...
			if (nxt_obj == null)	// next tkn is text; join must be word
				...
				switch (nxt_tid) {
					case Srch2_crt_tkn.Tid_space: case Srch2_crt_tkn.Tid_quote:
					case Srch2_crt_tkn.Tid_paren_bgn: case Srch2_crt_tkn.Tid_paren_end:
						join_is_word = false;	// next tkn is sym; and/or is not word; EX: a AND ; a AND"b"; a AND(b)
						break;
					case Srch2_crt_tkn.Tid_not: case Srch2_crt_tkn.Tid_and: case Srch2_crt_tkn.Tid_or:
		...
		if (txt_bgn != -1) {
			Tkns_add_word(Srch2_crt_tkn.Tid_word, txt_bgn, pos); // create word
			txt_bgn = -1;
		}
		return false;
	}
...
//		}
	private void Tkns_add_word(byte tid, int src_bgn, int src_end) {
		if (tkns.Count() > 0) {								// at least 1 tkn; check for "auto-and"
			Srch2_crt_tkn last_tkn = (Srch2_crt_tkn)tkns.Get_at_last();
			if (last_tkn.tid == Srch2_crt_tkn.Tid_word)	// previous tkn is word; auto "AND" words; EX: A B -> A AND B
				tkns.Add(new Srch2_crt_tkn(Srch2_crt_tkn.Tid_and, Bry_and));
		}
		if (tid == Srch2_crt_tkn.Tid_word) {				// if word has symbol, convert to quoted; EX: a-b should become "a-b"; otherwise searcher would search for a single word a-b
			...
			if (	words_len == 1					// only one word
				...
				tkns.Add(new Srch2_crt_tkn(tid, words[0]));
				return;
			}
			if (words.length > 1)					// multiple words; add as quoted-term; EX: "a-b"
				tid = Srch2_crt_tkn.Tid_word_quoted;
		}
		tkns.Add(new_tkn(tid, src, src_bgn, src_end));
	}
	private Srch2_crt_tkn new_tkn(byte tid, byte[] src, int val_bgn, int val_end) {return new Srch2_crt_tkn(tid, Bry_.Mid(src, val_bgn, val_end));}
	...
	.Add_str_byte(" "	, Srch2_crt_tkn.Tid_space)
	.Add_str_byte("\""	, Srch2_crt_tkn.Tid_quote)
	.Add_str_byte("-"	, Srch2_crt_tkn.Tid_not)
	.Add_str_byte("("	, Srch2_crt_tkn.Tid_paren_bgn)
	.Add_str_byte(")"	, Srch2_crt_tkn.Tid_paren_end)
	.Add_str_byte("or"	, Srch2_crt_tkn.Tid_or)
	.Add_str_byte("and"	, Srch2_crt_tkn.Tid_and);
}
