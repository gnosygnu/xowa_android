...
interface Srch2_sym_parser {
	int			Tid();
	byte[][]	Hooks_ary();
	int			Parse(Srch2_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end);
}
class Srch2_sym_parser_ {
	public static final int Tid__terminal = 1, Tid__split = 2, Tid__enclosure = 3, Tid__dot = 4, Tid__ellipsis = 5, Tid__apos = 6, Tid__dash = 7;
}
class Srch2_sym_parser__terminal implements Srch2_sym_parser {
	public Srch2_sym_parser__terminal(byte[] hook_bry) {this.hooks_ary = Bry_.Ary(hook_bry);}
	public int Tid() {return Srch2_sym_parser_.Tid__terminal;}
	public byte[][]	Hooks_ary() {return hooks_ary;} private final byte[][] hooks_ary;
	public int Parse(Srch2_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end) {
		if (mgr.Cur__end__chk(hook_end)) {	// hook at word_end; EX: "a, b" -> "a", "b"
			int word_bgn = mgr.Cur__bgn();
			if (word_bgn == Srch2_text_parser.None) return hook_end;
			int word_end = hook_bgn;
			for (int i = hook_bgn - 1; i >= word_bgn; --i) {	// loop bwd to gobble up streams of terminals; EX: "?!"
				byte b = src[i];
				if (mgr.terminal_trie.Match_bgn_w_byte(b, src, i, hook_bgn) == null) break;
				word_end = i;
			}
...
class Srch2_sym_parser__split implements Srch2_sym_parser {
	private final Btrie_slim_mgr trie = Btrie_slim_mgr.cs();
	private final boolean handle_eos;
	public Srch2_sym_parser__split(boolean handle_eos, String... hooks_ary_as_str) {
	...
	public int Tid() {return Srch2_sym_parser_.Tid__split;}
	public byte[][]	Hooks_ary() {return hooks_ary;} private final byte[][] hooks_ary;
	public int Parse(Srch2_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end) {
...
class Srch2_sym_parser__paren_bgn implements Srch2_sym_parser {
	private final byte bgn_byte, end_byte;
	public Srch2_sym_parser__paren_bgn(byte bgn_byte, byte end_byte) {
		this.bgn_byte = bgn_byte; this.end_byte = end_byte;
		this.hooks_ary = Bry_.Ary(Bry_.New_by_byte(bgn_byte));
	}
	public int Tid() {return Srch2_sym_parser_.Tid__enclosure;}
	public byte[][]	Hooks_ary() {return hooks_ary;} private final byte[][] hooks_ary;
	public int Parse(Srch2_text_parser mgr, byte[] src, int src_end, int paren_lhs_bgn, int paren_lhs_end) {
		int word_bgn = mgr.Cur__bgn();
		paren_lhs_end = Bry_find_.Find_fwd_while(src, paren_lhs_end, src_end, bgn_byte);
		int paren_rhs_bgn = Bry_find_.Find_fwd(src, end_byte, word_bgn + 1, src_end);
		...
		int word_end = mgr.Cur__end__find(paren_lhs_end + 1);
		if (word_end < paren_rhs_bgn)
			word_end = mgr.Cur__end__find(paren_rhs_end);
		...
		if (recurse)
			mgr.Make_copy().Parse(src, paren_lhs_end, paren_rhs_bgn);
		return word_end;
	}
}
class Srch2_sym_parser__dot implements Srch2_sym_parser {				// handle periods which will add two entries; EX: "H. G. Wells" -> "H.", "G.", "H", "G", "Wells"
	private final byte[] sym; private final int sym_len;
	public Srch2_sym_parser__dot(String sym_str) {this.sym = Bry_.new_u8(sym_str); this.sym_len = sym.length;}
	public int Tid() {return Srch2_sym_parser_.Tid__dot;}
	...
	public int Parse(Srch2_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end) {
		...
		if		(word_bgn == -1) {							// dot at start of word; EX: ".NET", "Colt .45"
			...
			mgr.Words__add(hook_bgn, word_end);				// make word; EX: ".45"
...
class Srch2_sym_parser__ellipsis implements Srch2_sym_parser {	// ellipsis which have variable length; EX: "..", "...", ".... "
	private final byte ellipsis_byte;
	public Srch2_sym_parser__ellipsis(byte ellipsis_byte, String hook) {
		this.ellipsis_byte = ellipsis_byte;
		this.hooks_ary = Bry_.Ary(hook);
	}
	public int Tid() {return Srch2_sym_parser_.Tid__ellipsis;}
	public byte[][]	Hooks_ary() {return hooks_ary;} private final byte[][] hooks_ary;
	public int Parse(Srch2_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end) {
		...
		mgr.Words__add(hook_bgn, rv);												// add ellipsis
		return rv;
	}
}
class Srch2_sym_parser__apos implements Srch2_sym_parser {	// apos which can be contraction ("I'm"), possessive ("today's") and plural ("plans'")
	private final byte[] hook_bry;
	public Srch2_sym_parser__apos(String hook) {
		this.hook_bry = Bry_.new_u8(hook);
		this.hooks_ary = Bry_.Ary(hook_bry);
	}
	public int Tid() {return Srch2_sym_parser_.Tid__apos;}
	public byte[][]	Hooks_ary() {return hooks_ary;} private final byte[][] hooks_ary;
	public int Parse(Srch2_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end) {
		int word_bgn = mgr.Cur__bgn();
		int word_end = -1;
		if		(word_bgn == -1) {							// sym at word_bgn; EX: "'n'"
			...
			mgr.Words__add(word_bgn, word_end);
			mgr.Words__add(word_bgn + 1, Bry_find_.Find_bwd__skip(src, word_end, word_bgn + 1, hook_bry));	// "'n'" -> "n"
			mgr.Cur__bgn__reset();
			return word_end;
		}
		...
		if (hook_end == word_end) {							// sym at word_end; EX: "A' "
			...
			if (root_word != null) {
				mgr.Words__add(word_bgn, word_end);			// add full; EX: "o'clock"
...
class Srch2_sym_parser__dash implements Srch2_sym_parser {
	public Srch2_sym_parser__dash(String hook) {this.hooks_ary = Bry_.Ary(hook);}
	public int Tid() {return Srch2_sym_parser_.Tid__dash;}
	public byte[][]	Hooks_ary() {return hooks_ary;} private final byte[][] hooks_ary;
	public int Parse(Srch2_text_parser mgr, byte[] src, int src_end, int hook_bgn, int hook_end) {
		int cur_bgn = mgr.Cur__bgn();				// get word_bgn
		if (cur_bgn != -1) {						// ignore "-" at start of word; EX: "-a"
			int dash_bgn = mgr.Dash__bgn();
			if (dash_bgn == Srch2_text_parser.None)	// 1st dash
				mgr.Words__add(cur_bgn, hook_bgn);	// add word; EX: "a" in "a-b"
	...
	public void Add_pending_word(Srch2_text_parser mgr, byte[] src, int word_end) {
		int dash_bgn = mgr.Dash__bgn();
		if (dash_bgn == Srch2_text_parser.None) return;
		mgr.Words__add_direct(dash_bgn, word_end);
		mgr.Dash__bgn_(Srch2_text_parser.None);
	}
}
