...
public class Srch_page_score_cmd extends Xob_cmd__base implements Xob_cmd {
	private int score_multiplier = 100000000;
	private boolean page_rank_enabled = false;
	public Srch_page_score_cmd(Xob_bldr bldr, Xowe_wiki wiki) {super(bldr, wiki);}
	public Srch_page_score_cmd Page_rank_enabled_(boolean v) {this.page_rank_enabled = v; return this;}
	@Override public String Cmd_key() {return Xob_cmd_keys.Key_search_page_score;}
	@Override public void Cmd_run() {
		...
		String page_rank_tbl = Srch_page_rank_cmd.Pagerank__tbl_name;
		...
		// percentize page_len_score to 0 : 100,000,000
		...
		new Sqlite_percentile_cmd(bldr, wiki).Init(Xob_db_file.Name__page_link, "temp_page_len", score_multiplier, String_.Concat_lines_nl_skip_last
		...
		if (page_rank_enabled) {
			...
			new Sqlite_percentile_cmd(bldr, wiki).Init(Xob_db_file.Name__page_link, "temp_page_score", 100000, String_.Concat_lines_nl_skip_last
			...
			( "UPDATE   page_rank_temp"
			, "SET      page_score = Cast(page_len AS int)"
			)));
		}
		...
		for (int i = 0; i < link_tbls_len; ++i) {
			...
			// update search_link.page_score
			link_tbl.conn.Meta_fld_assert(link_tbl.tbl_name, Srch_link_tbl.Fld_page_score, Dbmeta_fld_tid.Itm__int, 0);
			new Db_attach_mgr(link_tbl.conn
				, new Db_attach_itm("page_db", page_conn)
				).Exec_sql_w_msg
				( Bry_fmt.Make_str("updating search_link.page_score: ~{idx} of ~{total}", i + 1, link_tbls_len)
				, String_.Concat_lines_nl_skip_last
				( "UPDATE  search_link"
				, "SET     page_score = Coalesce((SELECT page_score FROM <page_db>page p WHERE p.page_id = search_link.page_id), 0)"
				));
			...
			new Db_attach_mgr(word_tbl.conn
				...
				( Bry_fmt.Make_str("updating search_word.word_page_score_max: ~{idx} of ~{total}", i + 1, link_tbls_len)
				, String_.Concat_lines_nl_skip_last
				( "REPLACE INTO search_word (word_id, word_text, word_page_count, word_page_score_max)"
				, "SELECT  sw.word_id, sw.word_text, sw.word_page_count, Max(sl.page_score)"
				, "FROM    search_word sw"
				, "        JOIN <link_db>search_link sl ON sl.word_id = sw.word_id"
				, "GROUP BY sw.word_id, sw.word_text, sw.word_page_count"
				, "HAVING  sw.word_page_score_max < Max(sl.page_score)"
				));

			// add search_link__word_id for perf
			link_tbl.conn.Meta_idx_create(Dbmeta_idx_itm.new_normal_by_tbl(link_tbl.tbl_name, "word_id", link_tbl.fld_word_id));
		}
	}
	@Override public Object Invk(GfsCtx ctx, int ikey, String k, GfoMsg m) {
		...
		return this;
	}	private static final String Invk__page_rank_enabled_ = "page_rank_enabled_";
}
