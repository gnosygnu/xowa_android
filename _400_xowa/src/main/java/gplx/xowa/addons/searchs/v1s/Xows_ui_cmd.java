...
public class Xows_ui_cmd implements GfoInvkAble, Cancelable, Srch_rslt_lnr, Xog_tab_close_lnr {
	...
	private Srch_rslt_reg cache;
	public Xows_ui_cmd(Xows_core mgr, Srch_qry qry, Xow_wiki wiki, Xoae_page page, Xog_tab_close_mgr tab_close_mgr, Xog_js_wkr js_wkr, Srch_rslt_reg cache, Srch_rslt_lnr async_wkr) {
	...
	public void Cancel_reset() {canceled = cancel_ackd = false;}
	public boolean Cancel_ackd() {return cancel_ackd;} public void Cancel_ackd_() {cancel_ackd = true;} private boolean cancel_ackd = false;
	public boolean Search() {
		...
		if (fill_from_cache) {
			...
			qry.page_max = cache.Total_count() / qry.page_len;
		}
		return rv;
	}
	public void Search_db() {
		...
		if (async) {
			...
			int qry_itms_bgn = qry.itms_bgn, cache_count = cache.Total_count();
		...
		cache.Clear();
		Srch2_rslt_hash rslts = search_mgr.Search(this, Srch2_rslt_cbk_.Noop, wiki, wiki.Lang().Case_mgr(), new Srch2_qry(qry.ns_mgr, wiki.Lang().Case_mgr().Case_build_lower(qry.search_raw), 0, 50));
		for (int i = 0; i < rslts.Len(); ++i) {
			Srch2_rslt_row row = rslts.Get_at(i);
			Srch_rslt_itm rslt_itm = new Srch_rslt_itm(wiki.Domain_bry(), wiki.Ttl_parse(row.Page_ns, row.Page_ttl_wo_ns), row.Page_id, row.Page_len, row.Page_score);
			if (cache.Has(rslt_itm.key)) continue;
			async_wkr.Notify_rslt_found(rslt_itm);
			cache.Add(rslt_itm);
		}
