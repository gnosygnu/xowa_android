...
public class Srch2_text_parser {
	...
	private Srch2_sym_parser__split parser__ws; private Srch2_sym_parser__dash parser__dash;
	public Srch2_word_hash word_hash = new Srch2_word_hash();
	public void Init_for_ttl(Xol_case_mgr case_mgr) {
		this.case_mgr = case_mgr;
		trie.Clear(); terminal_trie.Clear();
		parser__ws = new Srch2_sym_parser__split(Bool_.Y, " ", "\t", "\n", "\r", "_");
		parser__dash = new Srch2_sym_parser__dash("-");
		Parsers__reg(Parsers__make__terminal("!", "?", ",", ":", ";", "\""));
		Parsers__reg(new Srch2_sym_parser__split(Bool_.N, "/"));
		Parsers__reg(parser__ws, parser__dash
			, new Srch2_sym_parser__paren_bgn(Byte_ascii.Paren_bgn, Byte_ascii.Paren_end)
			, new Srch2_sym_parser__dot("."), new Srch2_sym_parser__ellipsis(Byte_ascii.Dot, ".."), new Srch2_sym_parser__apos("'"));	// NOTE: [ ] { } do not exist in titles
	}
	public Srch2_text_parser Make_copy() {
		Srch2_text_parser rv = new Srch2_text_parser();
	...
	public byte[][] Parse_to_bry_ary(byte[] src_orig) {
		Parse(Srch_text_parser_wkr__noop.Instance, src_orig);
		int hash_len = word_hash.Len();
		byte[][] rv = new byte[hash_len][];
		for (int i = 0; i < hash_len; ++i) {
			Srch2_word_itm itm = (Srch2_word_itm)word_hash.Get_at(i);
			rv[i] = itm.Word;
		}
		return rv;
	}
	public void Parse(Srch_text_parser_wkr wkr, byte[] src_orig) {
		word_hash.Clear();
		Parse(src_orig, 0, src_orig.length);
		int hash_len = word_hash.Len();
		for (int i = 0; i < hash_len; ++i) {
			Srch2_word_itm itm = (Srch2_word_itm)word_hash.Get_at(i);
			wkr.Parse_done(itm);
	...
	public void Parse(byte[] src_orig, int bgn, int end_orig) {
		this.src = case_mgr.Case_build_lower(src_orig); this.end = end_orig + (src.length - src_orig.length);
		this.cur_bgn = dash_bgn = Srch2_text_parser.None;
		int pos = bgn;
		while (true) {
			...
			if (o == null) {										// unknown sequence; word-char
				...
				Srch2_sym_parser parser = (Srch2_sym_parser)o;
	...
	private void Parsers__reg(Srch2_sym_parser... parsers_ary) {
		int parsers_len = parsers_ary.length;
		for (int i = 0; i < parsers_len; ++i) {
			Srch2_sym_parser parser = parsers_ary[i];
			...
			for (int j = 0; j < hooks_len; ++j) {
				...
				if (parser.Tid() == Srch2_sym_parser_.Tid__terminal)
					terminal_trie.Add_obj(hook, parser);
			}
		}
	}
	private static Srch2_sym_parser[] Parsers__make__terminal(String... hooks) {
		int rv_len = hooks.length;
		Srch2_sym_parser[] rv = new Srch2_sym_parser[rv_len];
		for (int i = 0; i < rv_len; ++i)
			rv[i] = new Srch2_sym_parser__terminal(Bry_.new_u8(hooks[i]));
		return rv;
	}
