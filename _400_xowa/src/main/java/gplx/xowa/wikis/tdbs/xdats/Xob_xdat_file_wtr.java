...
class SortAlgo_quick {// quicksort
	...
	private void Sort_recurse(int lo, int hi) {
		...
		while (i <= j) {												// divide into two lists
			while (comparer.compare(ary[i], mid) == CompareAble_.Less)	// if lhs.cur < mid, then get next from lhs	//#<>Compare(~compare(
				i++;				
			while (comparer.compare(ary[j], mid) == CompareAble_.More)	// if rhs.cur > mid, then get next from rhs	//#<>Compare(~compare(
				j--;
