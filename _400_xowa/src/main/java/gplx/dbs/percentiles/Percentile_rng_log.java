...
public class Percentile_rng_log {
	...
	public void Init(byte[] search, int rslts_needed, int slab_idx) {
		this.search = search; this.rslts_needed = rslts_needed; this.slab_idx = slab_idx;
		rdr_idx = -1;
	}
	public void Log(int score_bgn, int score_end, int rdr_found, int total_found, int pass_time) {
		fmtr.Log(search, rslts_needed, slab_idx, ++rdr_idx, score_bgn, score_end, score_end - score_bgn, rdr_found, total_found, pass_time);
	}
