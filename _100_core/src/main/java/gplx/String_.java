package gplx;
import java.lang.*;//#<>System.Text~java.lang
...
public class String_ implements GfoInvkAble {
	...
	public static String new_a7(byte[] v, int bgn, int end) {
		try {
			return v == null 
				? null
				: new String(v, bgn, end - bgn, "ASCII");		//#<>Encoding.ASCII.GetString(v, bgn, end - bgn)~new String(v, bgn, end - bgn, "ASCII")
		}
	...
	public static String new_u8(byte[] v, int bgn, int end) {
		try {
			return v == null
				? null
				: new String(v, bgn, end - bgn, "UTF-8");		//#<>Encoding.UTF8.GetString(v, bgn, end - bgn)~new String(v, bgn, end - bgn, "UTF-8")
		}
	...
	public static boolean Len_gt_0(String s)									{return s != null && s.length() >  0;}	//#<>.length~.length()
	public static boolean Len_eq_0(String s)									{return s == null || s.length() == 0;}	//#<>.length~.length()
	public static int Len(String s)											{return s.length();}						//#<>.length~.length()
	public static String Lower(String s)									{return s.toLowerCase();}					//#<>.ToLower~.toLowerCase
	public static String Upper(String s)									{return s.toUpperCase();}					//#<>.ToUpper~.toUpperCase
	public static String CaseNormalize(boolean caseSensitive, String s)		{return caseSensitive ? s : String_.Lower(s);}
	public static String Trim(String s)										{return s.trim();}						//#<>.Trim~.trim
	public static String Mid(String s, int bgn)								{return s.substring(bgn);}				//#<>.Substring~.substring
	public static String Replace(String s, String find, String replace)		{return s.replace(find, replace);}		//#<>.Replace~.replace
	public static char[] XtoCharAry(String s)								{return s.toCharArray();}				//#<>.ToCharArray~.toCharArray
	public static char CharAt(String s, int i)								{return s.charAt(i);}							//#<>s[i]~s.charAt(i)
	public static int CodePointAt(String s, int i)							{return s.codePointAt(i);}//#<>(int)char.GetNumericValue(s, i)~s.codePointAt(i)
	public static boolean Has(String s, String find)							{return s.indexOf(find) != String_.Find_none;}	//#<>.IndexOf~.indexOf
	public static boolean Has_at_bgn(String s, String v)						{return s.startsWith(v);}				//#<>.StartsWith~.startsWith
	public static boolean Has_at_end(String s, String v)						{return s.endsWith(v);}					//#<>.EndsWith~.endsWith
	public static int FindFwd(String s, String find)						{return s.indexOf(find);}				//#<>.IndexOf~.indexOf
	public static int FindFwd(String s, String find, int pos)				{return s.indexOf(find, pos);}			//#<>.IndexOf~.indexOf
	public static int FindBwd(String s, String find)						{return s.lastIndexOf(find);}			//#<>.LastIndexOf~.lastIndexOf
	...
	public static boolean Eq(String lhs, String rhs) {return lhs == null ? rhs == null : lhs.equals(rhs);} //#<>.Equals~.equals
	...
	public static boolean EqEmpty(String lhs) {return lhs.equals("");} //#<>.Equals~.equals
	public static String IfNullOrEmpty(String s, String or) {return s == null || s.length() == 0 ? or : s;}	//#<>.length~.length()
	...
	*/
	...
	public static int Compare_byteAry(String lhs, String rhs) {
		int lhsLen = lhs.length(), rhsLen = rhs.length();		//#<>.length~.length()
	...
	public static int[] XtoIntAry(String s, int len) {
		int[] rv = new int[len];
		for (int i = 0; i < len; i++)
			rv[i] = (int)s.charAt(i);		//#<>(int)s[i]~(int)s.charAt(i)
		return rv;
	}
	...
	public static String TrimEnd(String s) {if (s == null) return null;
		...
		int last = len;
		for (int i = len; i > 0; i--) {
			char c = s.charAt(i - 1);	//#<>s[i - 1]~s.charAt(i - 1)
			last = i;
			if (c != ' ' && c != '\t' && c != '\r' && c != '\n') {
				break;
			}
	...
	public static String Insert(String s, int pos, String toInsert) {
		...
		return s.substring(0, pos) + toInsert + s.substring(pos);//#<>s.Insert(pos, toInsert)~s.substring(0, pos) + toInsert + s.substring(pos)
	}
	...
	public static String Concat(String s1, String s2, String s3) {return s1 + s2 + s3;}//#<>String.Concat(s1, s2, s3)~s1 + s2 + s3
	...
	public static String[] Split_lang(String s, char c) {return s.split(Character.toString(c));}	//#<>s.Split(c)~s.split(Character.toString(c))
	...
	static String Mid_lang(String s, int bgn, int len) {return s.substring(bgn, bgn + len);}//#<>Substring(bgn, len)~substring(bgn, bgn + len)
